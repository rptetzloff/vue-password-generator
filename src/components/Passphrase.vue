                ...
                <v-card>
                    <v-card-text class="grey lighten-3">
                        <v-layout row wrap>
                            <v-flex xs12>
                                <v-switch
                                        v-model="subject_modifier"
                                        label="Adjective or Adverb (subject)"
                                ></v-switch>
                            </v-flex>
                        </v-layout>
                        ...
                        <v-layout row wrap>
                            <v-flex xs12>
                                <v-switch
                                        v-model="object_modifier"
                                        label="Adjective or Adverb (object)"
                                ></v-switch>
                            </v-flex>
                        </v-layout>
...
<script>
    ...
    import nouns from 'raw-loader!../../data/nouns.txt';
    import verbs from 'raw-loader!../../data/verbs.txt';
    import adjectives from 'raw-loader!../../data/adjectives.txt';
    ...
    export default {
        ...
        data: function () {
            return {
                ...
                subject_modifier: true,
                subject: true,
                verb: true,
                object_modifier: true,
                object: true,
        ...
        methods: {
            generatePassword: function () {
                ...
                let wordsFormula = [];
                if (this.subject_modifier) {
                    wordsFormula.push('subject_modifier');
                }
                if (this.subject) {
                    wordsFormula.push('subject');
                }
                if (this.verb) {
                    wordsFormula.push('verb');
                }
                if (this.object_modifier) {
                    wordsFormula.push('object_modifier');
                }
                if (this.object) {
                    wordsFormula.push('object');
                }

                if (wordsFormula.length < 1) {
                    this.sb_text = "You must select at least one word component.";
                    this.sb_color = 'error';
                    this.sb_state = true;
                } else {
                    //  First, we initialize the word list with the words from the word list.
                    let nounsList = this.loadWordList(nouns);
                    let verbsList = this.loadWordList(verbs);
                    let adjectivesList = this.loadWordList(adjectives);
                    // let wordList = nounsList.concat(verbsList).concat(adjectivesList);
                    ...
                    for (let i = 0; i < wordsFormula.length; i++) {
                        //  Get a random word from the word list.
                        let wordList = [];
                        switch (wordsFormula[i]) {
                            case 'subject_modifier':
                            case 'object_modifier':
                                wordList = adjectivesList;
                                break;
                            case 'subject':
                            case 'object':
                                wordList = nounsList;
